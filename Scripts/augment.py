#! /usr/bin/env python
'''
Based on Scott H. Hawley's audio augmenter:
...
def augment_audio(y, sr, n_augment = 0, allow_speedandpitch = True, allow_pitch = True,
    allow_speed = True, allow_dyn = True, allow_noise = True, tab="",quiet=False):
...
def augment_one_file(infile, n_augment, quiet):
    if os.path.isfile(infile):
        ...
        try:
          ...
          print(" *** File", infile, "failed to load: ", sys.exc_info()[0])
...
def main(args):
    ...
    if len(args.file) == 1 and os.path.isdir(os.path.normpath(args.file[0])):
        mypath=args.file[0]
        allFiles = []
        def walkDirectory(mypath):
            for (dirpath, dirnames, filenames) in os.walk(mypath):
                # print(dirpath, dirnames, filenames)
                allFiles.extend([os.path.join(dirpath, f) for f in filenames 
                    if f.lower().endswith(".wav") or f.lower().endswith(".aif")])
                for d in dirnames:
                    walkDirectory(os.path.join(dirpath, d))
        walkDirectory(mypath)
        args.file = allFiles

    # read in every file on the list, augment it lots of times, output all those
    for file in args.file:
        augment_one_file(file, args.N, args.quiet)
...
if __name__ == "__main__":
    ...
    parser.add_argument('file', help="sound files to augment or a single path which is scanned recursively", nargs='*')
