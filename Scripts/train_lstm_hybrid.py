"""
  Train Keras LSTM model on the time series data and statistics with an ANN on separate branches
...
from keras.layers import concatenate, Input, Dense, Dropout, CuDNNLSTM, Bidirectional
...
for i in range(len(X_train_sets)):
    ...
    # plt.imshow(X_train_time_series[100])
    # plt.imshow(X_train_statistics[100])
    # plt.show()
    ...
    ### left branch: LSTM on time series
    ...
    x2 = Dense(300, activation='elu')(x2_input)
    x2 = Dropout(0.5)(x2)

    ### final model: merge LSTM and ANN
    ...
    model_filepath = os.path.join(args['classifier_root_path'], 'model_lstm_split.hdf5')
    modelCheckpoint = ModelCheckpoint(filepath=model_filepath, 
        monitor='val_loss', save_best_only=True)

    history = model.fit(
        [X_train_time_series, X_train_statistics], 
        y_train, 
    ...
    # evaluate
    score, acc = model.evaluate([X_test_time_series, X_test_statistics], y_test)
